...
interface
  ...
  TBsonBuffer = class(TObject)
    ...
    public
      ...
      function AppendStr(name : string; value : string) : Boolean;  //¡˙ À‘∆ 2012-11-27
      ...
      function append(name : PAnsiChar; var value : Double) : Boolean; overload;
      { Append a TDateTime to the buffer; converted to 64-bit POSIX time }
      function AppendDateTime(name : PAnsiChar; value : TDateTime) : Boolean; //¡˙ À‘∆

      function append(name : PAnsiChar; var value : TDateTime) : Boolean; overload;
...
implementation
  ...
  function TBsonBuffer.AppendStr(name: string; value: string) : Boolean;
  begin
    if handle = nil then
      raise Exception.Create('BsonBuffer already finished');
    Result := append(PChar(name),PChar(Value));
  end;
  ...
  function TBsonBuffer.append(name: PAnsiChar;var  value: Double) : Boolean;
  begin
    if handle = nil then
      raise Exception.Create('BsonBuffer already finished');
  ...
  function TBsonBuffer.AppendDateTime(name: PAnsiChar; value: TDateTime) : Boolean;
  begin
    if handle = nil then
      raise Exception.Create('BsonBuffer already finished');
  ...
  function TBsonBuffer.append(name: PAnsiChar;var  value: TDateTime) : Boolean;
  begin
    if handle = nil then
      raise Exception.Create('BsonBuffer already finished');
    Result := (bson_append_date(handle, name, Trunc((value - 25569) * 1000 * 60 * 60 * 24)) = 0);
  ...
  begin
    case VarType(value) of
      ...
      varDate: Result := AppendDateTime(name, TDateTime(value));
      varInt64: Result := append(name, TVarData(value).VInt64);
